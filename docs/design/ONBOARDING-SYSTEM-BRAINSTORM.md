...
Comprehensive onboarding system for all organization types in the ABY-MED multi-tenant platform with CSV import capabilities, QR code generation, and data management.
...
- id (UUID)
- name (TEXT)
- org_type (manufacturer|supplier|distributor|dealer|hospital|service_provider|other)
...
- rel_type (manufacturer_of|distributor_of|dealer_of|supplier_of|partner_of)
- metadata (JSONB)
```
...
- equipment_id (→ equipment_catalog)
...
## 🏢 Organization Type Onboarding Requirements
...
### **4. DISTRIBUTOR/DEALER Onboarding**
...
## 📥 CSV Import System - Current State & Extensions
...
**Issues Identified:**
- ❌ No manufacturer_id field (needs to be added)
- ❌ No equipment_catalog_id field
- ❌ No organization_id for customer
- ❌ No QR code pre-generation
- ❌ No bulk QR code generation before assignment
- ❌ No photo/document upload via CSV
- ❌ No specifications in structured format
...
├── Create organization record (type=manufacturer)
├── Add headquarters facility
├── Upload company documents
└── Set status=pending_verification

Step 2: Admin User Creation
├── Create admin user account
├── Link to organization (org_admin role)
├── Auto-generate password
├── Send welcome email with setup link
└── Enable MFA setup (optional)

Step 3: Equipment Catalog Import
├── Prepare equipment catalog CSV
├── Import equipment types via CSV
├── Upload product images
├── Upload manuals & brochures
└── Verify and activate catalog items

Step 4: Parts Catalog Import
├── Prepare parts catalog CSV
├── Import parts linked to equipment
├── Set pricing and lead times
└── Mark critical parts

Step 5: Service Contacts Setup
├── Import technical contacts CSV
├── Import billing contacts CSV
├── Set primary contacts
└── Verify contact details

Step 6: Facilities Setup
├── Import facilities/branches CSV
├── Set geographic coverage
└── Assign service territories

Step 7: QR Code Generation
├── Bulk generate QR codes for equipment types
├── Download QR code sheets (PDF)
├── Print and apply to equipment
└── Track QR code usage

Step 8: Equipment Registry (Optional)
├── Import installed equipment CSV
├── Link to equipment catalog
├── Assign to customer organizations
├── Set warranty and service dates
└── Upload installation photos

Step 9: Verification & Activation
├── Admin reviews all data
├── Verifies certifications
├── Approves manufacturer account
└── Status → active
```
...
```
Step 1: Organization Creation
├── Create service provider org
├── Set service areas
└── Upload certifications

Step 2: Engineers Bulk Import
├── Prepare engineers CSV
├── Import engineer profiles
...
├── Set authorized service status
└── Configure warranty servicing
...
1. Add missing fields to equipment_registry
   - manufacturer_id (UUID → organizations)
   - organization_id for customer (UUID → organizations)
...
1. ✅ Equipment Catalog CSV Import
2. ✅ Equipment Parts CSV Import
3. ✅ Enhanced Equipment Registry Import
4. ✅ QR Code Bulk Generation
5. ✅ Organizations Bulk Import
6. ✅ Users Bulk Import
...
7. ⚠️ Organization Contacts Import
8. ⚠️ Engineers Bulk Import
9. ⚠️ Facilities Import
10. ⚠️ QR Code PDF Generation

### **Low Priority (Nice to Have)**
11. 🔵 Organization Relationships Import
12. 🔵 QR Code Branding/Customization
13. 🔵 Advanced validation rules
14. 🔵 Auto-detection of CSV format

...
export const equipmentApi = {
  ...
  async getByQRCode(qrCode: string) {
    try {
      const response = await apiClient.get<Equipment>(`/api/v1/equipment/qr/${qrCode}`);
      return response.data;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  ...
  async getBySerial(serialNumber: string) {
    try {
      const response = await apiClient.get<Equipment>(`/api/v1/equipment/serial/${serialNumber}`);
      return response.data;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  ...
  async update(id: string, data: Partial<Equipment>) {
    try {
      const response = await apiClient.patch<{ message: string }>(`/api/v1/equipment/${id}`, data);
      return response.data;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  ...
  async generateQRCode(id: string) {
    try {
      const response = await apiClient.post<{ message: string; path: string }>(`/api/v1/equipment/${id}/qr`);
      return response.data;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  ...
  async bulkGenerateQRCodes() {
    try {
      const response = await apiClient.post<{ 
        ...
        failed_ids?: string[];
        message: string 
      }>('/api/v1/equipment/qr/bulk-generate');
      return response.data;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
  ...
  async downloadQRLabel(id: string) {
    try {
      const response = await apiClient.get(`/api/v1/equipment/${id}/qr/pdf`, {
        responseType: 'blob',
      });
  ...
  async importCSV(file: File, createdBy: string) {
    try {
      ...
      const response = await apiClient.post<CSVImportResult>('/api/v1/equipment/import', formData, {
        headers: {
  ...
  async recordService(id: string, data: { service_date: string; notes: string }) {
    try {
      const response = await apiClient.post<{ message: string }>(`/api/v1/equipment/${id}/service`, data);
      return response.data;
    } catch (error) {
      throw new Error(handleApiError(error));
    }
